...
public class LinkedList : IEnumerable<int> {
    ...
    public void Remove(int value) {
        Node? curr = _head;
        while (curr is not null) { // loop through the linked list
            if (curr.Data == value) { // check if the current node contains the value
                if (curr == _head) {
                    RemoveHead();
                } else if (curr == _tail) {
                    RemoveTail();
    ...
    public void Replace(int oldValue, int newValue) {
        Node? curr = _head;
        while (curr is not null) {
            if (curr.Data == oldValue) { // Check if the current node contains the old value
                curr.Data = newValue; // Replace the current node with the new value
            }
            curr = curr.Next;
        }
    }
